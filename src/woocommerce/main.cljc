(ns woocommerce.main
  "Kotodama WASM entrypoint for the Woocommerce clean-room actor (L5) — Clojure port.
  ...
  facts (`woocommerce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "woocommerce")
(def tier "L5")
...
(def entity-specs
  [{:entity "Order"           :plural "orders"           :id-prefix "woocomme_ord"
    :fields [:number :orderKey :status :currency :total :totalTax :discountTotal :customerId :dateCreated]
    :required [:number]
    :coerce {:total :float :totalTax :float :discountTotal :float :customerId :int}
    :refs {:customerId "Customer"}}
   {:entity "Product"         :plural "products"         :id-prefix "woocomme_pro"
    :fields [:name :slug :type :status :featured :catalogVisibility :sku :regularPrice :salePrice :onSale :stockQuantity :stockStatus :virtual :downloadable]
    :required [:name]
    :coerce {:featured :bool :regularPrice :float :salePrice :float :onSale :bool :stockQuantity :int :virtual :bool :downloadable :bool}
    :refs {}}
   {:entity "Customer"        :plural "customers"        :id-prefix "woocomme_cus"
    :fields [:email :firstName :lastName :username :role :isPayingCustomer :dateCreated]
    :required [:email]
    :coerce {:isPayingCustomer :bool}
    :refs {}}
   {:entity "Coupon"          :plural "coupons"          :id-prefix "woocomme_cou"
    :fields [:code :amount :discountType :description :individualUse :freeShipping :minimumAmount]
    :required [:code]
    :coerce {:amount :float :individualUse :bool :freeShipping :bool :minimumAmount :float}
    :refs {}}
   {:entity "ProductCategory" :plural "productcategories" :id-prefix "woocomme_cat"
    :fields [:name :slug :parent :description :display :menuOrder]
    :required [:name]
    :coerce {:parent :int :menuOrder :int}
    :refs {}}
   {:entity "Refund"          :plural "refunds"          :id-prefix "woocomme_ref"
    :fields [:amount :reason :refundedBy :refundedPayment :dateCreated]
    :required [:amount]
    :coerce {:amount :float :refundedBy :int :refundedPayment :bool}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"woocommerce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "woocommerce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :woocommerce-compat/ready)
